...
describe("onboardOpenClaw", () => {
  it("persists the shared onboarding side effects through package services", async () => {
      ...
      },
      {
        ...
        fetchGatewayCronJobs: async (gatewayUrl: string, gatewayToken: string) => {
          ...
          return [
            ...
            { id: "cron-5", name: "Stringified cron", schedule: JSON.stringify("0 0 * * *"), enabled: true },
            { id: "cron-6", name: "Stringified object", schedule: JSON.stringify({ every: "30m" }), enabled: true },
          ];
        },
    ...
    assert.equal(result.cronJobs.length, 6);
    ...
    assert.equal(jobs[4].scheduleKind, "cron");
    assert.equal(jobs[4].scheduleExpr, "0 0 * * *");
    assert.equal(jobs[5].scheduleKind, "every");
    assert.equal(jobs[5].scheduleExpr, "30m");
  });
});
